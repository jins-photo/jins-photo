    ...
    <div className="min-h-screen bg-black text-white p-4 font-sans">
      ...
      <div className="flex justify-between items-center mb-6 bg-zinc-900/90 p-4 rounded-3xl border border-white/10 sticky top-0 z-[100] backdrop-blur-md">
        ...
        <div className="flex gap-2 text-[11px] font-bold">
          {auth.role === 'admin' ? (
            <button onClick={() => { sessionStorage.clear(); window.location.reload(); }} className="bg-zinc-800 px-4 py-2 rounded-2xl text-zinc-500">로그아웃</button>
          ) : (
            <button onClick={() => { if(prompt("마스터 비번?") === config.masterPw) { setAuth({role:'admin'}); sessionStorage.setItem('photo_auth', JSON.stringify({role:'admin'})); window.location.reload(); }}} className="bg-zinc-800 px-4 py-2 rounded-2xl">관리자 로그인</button>
          )}
        </div>
      </div>
        ...
        {items.folders.map(f => {
          ...
          return (
            <div key={f} className="relative h-36 bg-zinc-900 rounded-[2.5rem] border border-zinc-800 flex flex-col group overflow-hidden">
              ...
              {/* 우상단 메뉴 버튼 (관리자 전용) */}
              {auth.role === 'admin' && (
                <button onClick={(e) => { e.stopPropagation(); setActiveFolderMenu(activeFolderMenu === f ? null : f); }} className="absolute right-2 top-0 w-10 h-10 flex items-center justify-center text-zinc-500 text-3xl font-black z-30">︙</button>
              )}
              ...
              {/* 관리자 전용 메뉴 팝업 */}
              {activeFolderMenu === f && (
                <div className="absolute right-3 top-10 w-44 bg-zinc-800 rounded-2xl z-[110] border border-zinc-700 shadow-2xl p-1 font-bold">
                  <button onClick={() => { handleShare(f, true); setActiveFolderMenu(null); }} className="w-full p-4 text-[11px] text-left hover:bg-zinc-700 border-b border-zinc-700 text-white">🔗 폴더 공유</button>
                  <button onClick={() => {
                    const p = prompt("외부인용 비번 설정 (공백은 해제)");
                    const nc = { ...config, folderPws: { ...config.folderPws, [fp]: p || "" } };
                    updateConfig(nc);
                  }} className="w-full p-4 text-[11px] text-left hover:bg-zinc-700 border-b border-zinc-700 text-white">🔑 비번 설정</button>
                  <button onClick={() => { if(confirm("삭제할까요?")) fetch(`${WORKER_URL}?path=${fp}/`, { method: 'DELETE' }).then(() => window.location.reload()); }} className="w-full p-4 text-[11px] text-left text-red-400">🗑️ 삭제</button>
                </div>
              )}
            </div>
          );
