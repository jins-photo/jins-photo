...
export default function Page() {
  ...
  // [핵심: 링크 복사 함수]
  const copyLink = (name: string, isFolder: boolean) => {
    const fullUrl = `${window.location.origin}?path=${currentPath}${name}${isFolder ? "/" : ""}`;
    ...
    <div style={{ backgroundColor: 'black', minHeight: '100vh', color: 'white', padding: '20px', fontFamily: 'sans-serif' }}>
            ...
            <div key={f} style={{ position: 'relative' }}>
              ...
              {activeMenu === f && (
                <div style={{ position: 'absolute', top: '40px', right: '5px', background: '#27272a', borderRadius: '12px', zIndex: 100, padding: '5px', width: '150px', border: '1px solid #333' }}>
                  ...
                  <button onClick={() => copyLink(f, true)} style={{ width: '100%', textAlign: 'left', background: 'none', border: 'none', color: 'white', padding: '10px', fontSize: '12px', borderTop: '1px solid #222' }}>🔗 폴더 공유</button>
          ...
          <div key={file} style={{ position: 'relative' }}>
            ...
            {activeMenu === file && (
              <div style={{ position: 'absolute', top: '35px', right: '0', background: '#27272a', borderRadius: '12px', zIndex: 100, padding: '5px', width: '130px', border: '1px solid #333' }}>
                ...
                <button onClick={() => copyLink(file, false)} style={{ width: '100%', textAlign: 'left', background: 'none', border: 'none', color: 'white', padding: '10px', fontSize: '12px', borderTop: '1px solid #222' }}>🔗 사진 공유</button>
      ...
      {selectedImg && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.98)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setSelectedImg(null)}>
          ...
          <div style={{ marginTop: '20px', display: 'flex', gap: '10px' }} onClick={(e)=>e.stopPropagation()}>
             <button onClick={() => copyLink(selectedImg!, false)} style={{ background: '#3b82f6', color: 'white', padding: '12px 25px', borderRadius: '15px', border: 'none', fontWeight: 'bold' }}>🔗 공유</button>
